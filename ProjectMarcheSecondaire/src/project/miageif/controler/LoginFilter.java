...
public class LoginFilter implements Filter{
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (auth == null || auth.getStatus().equals("DisConnected")) {
        	res = (HttpServletResponse) response; 
        	System.out.println("******** "+req.getContextPath() + "logout");
        	res.sendRedirect(req.getContextPath() + "/pages/public/accueil.xhtml"); // No logged-in user found, so redirect to login page.
        } else {
